...
\]
Experiments use $\tau_t=0.2$. Moment recursions are exact, so validation has no trajectory noise.
\end{block}
...
\end{exhibit}
\exhibitnote{Mean and one standard deviation over five seeds at $T=10$ with $\tau=0.2$. Higher objective is better.}
...
\midrule
Optimum & $-13.154$ & $0$ & $0.000$ & -- \\
REINFORCE & $-20.016{\pm}0.001$ & $6.862$ & $1.384{\pm}0.000$ & $224$ \\
Transport, $\lambda=0.2$ & $-13.605{\pm}0.200$ & $0.451$ & $0.236{\pm}0.059$ & $796$ \\
Transport, $\lambda=0.4$ & $\mathbf{-13.537{\pm}0.115}$ & $\mathbf{0.384}$ & $\mathbf{0.214{\pm}0.038}$ & $795$ \\
\bottomrule
\end{tabular}
\end{exhibit}
\exhibitnote{Mean $\pm$ standard deviation over five seeds at $T=10$ with $\tau=0.2$. Gap and wall-clock time are reported as seed averages.}

\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}{Portfolio: terminal wealth diagnostics}
\begin{exhibit}{Terminal law under the learned policies}
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{@{}lrrr@{}}
\toprule
Method & $\E[X_T]$ & $\operatorname{Var}(X_T)$ & $\E[X_T]-\E[X_0]$ \\
\midrule
Optimum & $0.543$ & $0.388$ & $-0.457$ \\
REINFORCE & $1.040{\pm}0.000$ & $0.026{\pm}0.000$ & $+0.040{\pm}0.000$ \\
Transport, $\lambda=0.2$ & $0.535{\pm}0.017$ & $0.414{\pm}0.024$ & $-0.465{\pm}0.017$ \\
Transport, $\lambda=0.4$ & $0.542{\pm}0.020$ & $0.403{\pm}0.020$ & $-0.458{\pm}0.020$ \\
\bottomrule
\end{tabular}
\end{exhibit}
\exhibitnote{Exact terminal moments from the deterministic moment recursion, reported as mean $\pm$ standard deviation over five seeds. Initial mean wealth is $\E[X_0]=1$.}

\vspace{0.2cm}
This benchmark is not pure wealth maximization. With $\gamma=2$, the social objective penalizes high population mean throughout the trajectory, so the optimal policy itself has terminal mean below the initial wealth. The algorithm ``works'' by optimizing this mean-field objective, not by maximizing raw terminal wealth.
\end{frame}
