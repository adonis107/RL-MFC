...
\includegraphics[width=0.78\linewidth]{portfolio_validation_compact_T10.png}
\end{exhibit}
\exhibitnote{Mean and one standard deviation over five seeds at $T=10$. Higher objective is better.}

\vspace{0.1cm}
Adaptive transport keeps improving throughout training. Fixed transport also improves, while standard REINFORCE does not recover the missing mean-field direction.
\end{frame}
...
\midrule
Optimum & $-13.133$ & $0$ \\
REINFORCE & $-20.535$ & $7.402$ \\
Best fixed transport & $-17.980$ & $4.847$ \\
Adaptive transport & $\mathbf{-17.193}$ & $\mathbf{4.060}$ \\
\bottomrule
\end{tabular}
\end{exhibit}
\exhibitnote{Mean over five seeds at $T=10$. Higher objective is better; the gap is $|J(\widehat\theta)-J(\theta^\star)|$.}

\vspace{0.2cm}
At the same simulator budget, adaptive transport ends about $3.34$ objective units closer to the closed-form optimum than REINFORCE. The fixed-scale transport runs already improve substantially; the adaptive scale gives the cleanest headline number.
\end{frame}
