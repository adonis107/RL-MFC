...
\textbf{Related work.}
\begin{itemize}
  \item \textbf{Model-based MFC:} dynamic programming, forward--backward HJB--Fokker--Planck systems, or direct differentiation through the population dynamics \cite{pham2018bellman,carmona2018probabilistic,lauriere2021numerical}.
  \item \textbf{Reinforcement learning for MFC:} actor--critic methods reduce model dependence but still use a critic or structural information \cite{frikha2025actor,pham2025actor}.
  \item \textbf{Fully model-free policy gradients:} available in discrete time on finite state spaces by perturbing the logits of the population vector \cite{meunier2026model}. This work separates that idea from the logit representation.
\end{itemize}
...
\[
  ...
  \right],
\]
where $\tau=(X_0,\alpha_0,\ldots,X_T)$ and $R(\tau)=\sum_{t=0}^{T-1}r_t(X_t,\alpha_t)+g(X_T)$.

\vspace{0.2cm}
This is model-free because, conditionally on $(X_t,\alpha_t)$, the transition kernel does not depend on $\theta$, so it factors out of the trajectory density before differentiation. The estimator uses sampled rewards and policy scores, and never evaluates or differentiates $P_t$.

\vspace{0.3cm}
In mean-field control the kernel reads $\mu_t^\theta$, and $\mu_t^\theta$ is generated by the policy, so it does not factor out.

\end{frame}

\begin{frame}{Closest Existing Construction}
\small
For finite state spaces, Meunier et al.~\cite{meunier2026model} work with strictly positive population vectors and use the logit chart
\[
...
\vspace{0.25cm}
The limitation is representation: one logit coordinate per state. This is natural on a finite state space but has no direct analogue for a general probability measure on a continuous state space.

\vspace{0.15cm}
\begin{block}{Objective}
  The goal is to construct a more explicable perturbation framework, which could extend to continous state spaces.
\end{block}
\end{frame}
...
\[
  X_0^{\lambda,\theta}\sim\mu_0,\qquad
  \alpha_t^{\lambda,\theta}\sim\pi_t^\theta(\cdot\mid X_t^{\lambda,\theta},M_t^{\lambda,\theta}),\qquad
  X_{t+1}^{\lambda,\theta}\sim P_t(\cdot\mid X_t^{\lambda,\theta},M_t^{\lambda,\theta},\alpha_t^{\lambda,\theta}),
\]
...
\vspace{0.05cm}
The objective estimate gives consistency of optimizers: points of maximizers of $J^\lambda$ maximize $J$. So the perturbed problem is a legitimate surrogate, and its gradient is the right target.
\end{frame}
...
Charts used:
\begin{itemize}
  \item the simplex when $\Xc$ is finite,
  \item the logit map of Meunier et al.,
  \item a Gaussian-mixture decoder in the theory,
  \item and the exact moment map $\mu\mapsto\E_\mu[X]$ in the experiments.
\end{itemize}
\end{frame}
...
\begin{block}{Cost per policy update}
  \begin{center}
  \scriptsize
  \begin{tabular}{lccc}
  \toprule
   & REINFORCE & MF-REINFORCE & Transport \\
  \midrule
  Complexity & $BT$ & $BT+nT(T+1)$ & $T(B_{\rm tr}+n_{\rm tr})$ \\
  \bottomrule
  \end{tabular}
  \end{center}
\end{block}
...
\begin{tabular}{lrrrr}
\toprule
& \multicolumn{2}{c}{$J(\theta^\star)-J(\hat\theta)$} & \multicolumn{2}{c}{Policy error} \\
...
\vspace{0.1cm}
\alert{These use exact moment-charts.} In both, the perturbed argument is the population mean, so the chart is exact, one-dimensional, and $\delta_K^\theta=0$. No Gaussian mixture is fitted anywhere. The estimator is built by analogy with the finite-state case; its guarantees here are an extrapolation.
...
\small
Four predictions of the analysis are testable from the saved artifacts.
...
\vspace{0.05cm}
\small{
\alert{Evaluation-only. Not part of the model-free estimator: it needs a differentiable evaluation path.}
...
\begin{frame}{What Is Proved, What Is Implemented, What Is Open}
\scriptsize
...
Estimator of the flow sensitivity      & \good{proved} & \bad{open} \\
...
\begin{itemize}
  ...
  \item \textbf{Testable using alignment:} the alignment diagnostic costs two AD passes and no training grid, wherever a differentiable evaluation path exists.
  \item \textbf{Predictions checked against the runs:} the theory is validated numerically.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{The adaptive controller can be improved:} it reads unresolved bias as no bias.
...
\vspace{0.25cm}
\textbf{New benchmarks.} Fourier modes in Kuramoto-type synchronization: a two-dimensional chart.
...
\vspace{0.15cm}
\scriptsize
Closest existing work: the actor--critic algorithms for mean-field control of Pham--Warin~\cite{pham2025actor}, where the critic is a network reading moments of the population law.
\end{frame}
