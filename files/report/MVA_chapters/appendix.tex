...
\input{tables/budget_runtime}

\section{Full objective tables}
\label{appendix:full-objectives}

\input{tables/full_objectives_twostate}

\input{tables/full_objectives_cybersecurity}

\input{tables/full_objectives_distribution}

\input{tables/full_objectives_advertising}

\input{tables/full_objectives_lq}

\input{tables/full_objectives_portfolio}
...
\input{tables/twostate_policy_error}

\section{The two-state calibration grid}
\label{appendix:twostate-grid}

Table~\ref{tab:twostate-grid} is the full $(\lambda,\eta)$ sweep on which the auxiliary scale used by every other benchmark was calibrated, and Figure~\ref{fig:twostate-eta} shows the effect of $\eta$ at a fixed main scale, with and without a particle approximation of the population flow.

\input{tables/twostate_grid}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.49\linewidth]{figures/distribution_validation_T5.png}
\includegraphics[width=0.49\linewidth]{figures/distribution_terminal_law.png}
\caption{Distribution-planning benchmark at $T=5$. Left: validation reward against training iteration, as mean over five seeds with one standard deviation shaded; REINFORCE is the flat line at $-0.2616$. Right: terminal population law learned by fixed-scale transport at $\lambda=0.4$, against the uniform initial law and the target. The learned flow is coherent transport towards the target mode rather than local oscillation, but it overshoots the centre and undershoots the shoulders.}
...
The equal-budget rule of Section~\ref{subsec:experimentation-protocol} fixes the total number of simulated transitions per update but not their split between the main trajectories and the auxiliary sensitivity estimate. This section records that split for every benchmark, and the two experiments that test whether reallocating it changes the outcome.

\paragraph{The auxiliary budget is the binding constraint.}
The equal-budget rule of Section~\ref{subsec:experimentation-protocol} fixes the total $T(B_{\mathrm{tr}}+n_{\mathrm{tr}})$ but leaves the split free, and Table~\ref{tab:auxiliary-budget} shows that the split is where this benchmark differs from the rest of the suite. The transport runs use $B_{\mathrm{tr}}=549$ main trajectories and $n_{\mathrm{tr}}=11$ auxiliary trajectories, while the population argument is a ten-point simplex whose derivative has $d=9$ free coordinates. That is $1.2$ auxiliary trajectories per coordinate, against $12$ on the two-state and advertising problems, $20$ on the linear--quadratic benchmark and $200$ on the portfolio benchmark. Distribution planning is the only benchmark on which the auxiliary estimate is asked to resolve more coordinates than it has samples, and it is also the benchmark on which transport underperforms MF-REINFORCE most.

\input{tables/auxiliary_budget}

The reallocation is available in principle, since the budget rule constrains only the sum: taking $B_{\mathrm{tr}}=200$ and $n_{\mathrm{tr}}=360$ leaves $T(B_{\mathrm{tr}}+n_{\mathrm{tr}})=2\,800$ unchanged and raises the auxiliary sample size to $40$ per coordinate. It is also affordable in arithmetic. One might expect the cost of the update to grow linearly in $n_{\mathrm{tr}}$, but the sensitivity recursion is applied to the whole auxiliary batch at once, and a direct measurement gives a factor $1.9$ in seconds per training step when $n_{\mathrm{tr}}$ goes from $11$ to $360$, a $33$-fold increase in auxiliary samples. The dominant cost of the update on this benchmark is the exact population recursion over the ten-point simplex, which the reallocation does not touch.

\input{tables/reallocation}

The same reasoning does not apply to targeted advertising. Its population argument is a two-point simplex with a single free coordinate, and its transport runs already use $n_{\mathrm{tr}}=12$ auxiliary trajectories, the same ratio as the two-state benchmark on which transport is the best estimator at $T=5$. The auxiliary budget is therefore not what limits it, and Section~\ref{subsec:targeted-advertising} identifies a different mechanism.

\section{Theoretical-validation tables}
\label{appendix:theory-tables}
\label{appendix:tv-bounds}

These are the three measurement tables behind Table~\ref{tab:theory-verdicts} of Section~\ref{sec:theory-validation}: the perturbation bound along every finite-state transport flow, the objective bias against the perturbation scale on the two benchmarks with a closed-form optimum, and the replicated gradient diagnostics on the linear--quadratic benchmark.

\input{tables/tv_bounds}
...
\input{tables/gradient_diagnostics_portfolio}
