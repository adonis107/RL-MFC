...
\begin{align}
    \pi_t^\theta(\d a\mid x) = p_t^\theta(a\mid x)\nu_\Ac(\d a).
\end{align}
...
\end{align}
Subtracting the baseline therefore preserves unbiasedness while potentially reducing variance. A common choice is the conditional expected cost-to-go $b_t(x)=\E^{\pi^\theta}[C_t\mid X_t=x]$, although it is not in general the variance-minimizing baseline{greensmith2004variance}. Estimating this quantity jointly with the policy leads to actor--critic methods. The present work instead builds on the score-function structure of REINFORCE.
...
A related approach is based on cylindrical representations
\begin{align}
    \Phi_J(m) = \left(\langle\varphi_1,m\rangle,\ldots,\langle\varphi_J,m\rangle \right),
\end{align}
