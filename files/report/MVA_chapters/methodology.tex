...
\begin{theorem}[Perturbation estimate]
    ...
    Let $\lambda\in(0,1)$ and let $q_t\in\Delta_N$. For the perturbed population law $M_t^{\lambda,\theta}(\omega) = (1-\lambda)\mu_t^\theta + \lambda q_t(\omega)$, we have $d_\mathrm{TV}(M_t^{\lambda,\theta}(\omega), \mu_t^\theta) \leq \lambda, \qquad \text{for every }\omega$.
...
\end{align}
and the averaged running reward $R_t^\theta(i,m)\coloneqq \int_\Ac r_t(i,m,a)\pi_t^\theta(\d a\mid i,m),\qquad i\in\Xc,\ m\in\Delta_N$.
...
\begin{corollary}[Convergence of optimal values and policies]
    ...
    \begin{align}
        V &\coloneqq \sup_{\theta\in\Theta} J(\theta), \\
        V_K^\lambda &\coloneqq \sup_{\theta\in\Theta} J_K^\lambda(\theta).
    \end{align}
    Then
    \begin{align}
